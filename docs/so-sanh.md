...
| Kỹ thuật | Ở bản gốc | Ở shark_pid |
|---|---|---|
| Tích phân hình thang | `f_Trapezoid_Intergral` | `SHARK_PID_F_TRAPEZOID_I` |
| Biến tốc độ tích phân A/B | `f_Changing_Integral_Rate` | `cfg.ci_a`, `cfg.ci_b` |
| Vi phân theo giá trị đo | `f_Derivative_On_Measurement` | `cfg.c = 0` |
| Lọc khâu D | `f_Derivative_Filter` | `cfg.d_tau` |
| Lọc ngõ ra | `f_Output_Filter` | `cfg.out_tau` |
| Phát hiện kẹt cơ cấu | `f_PID_ErrorHandle` | `cfg.stall_*` |
...
shark_pid không dùng macro cho việc này — chỉ `fabsf` và hàm `static`.
...
shark_pid đếm bằng `stall_time` (giây), dùng `fabsf` nên bắt được cả hai chiều,
không có phép chia nào, và `shark_pid_clear_status()` xoá được.
...
shark_pid xét dấu của **lượng bị cắt ở ngõ ra** (`u_raw - u`), là trạng thái bão
hoà thực. Và có thêm back-calculation như lựa chọn mượt hơn.
...
- **Giới hạn dốc ngõ ra** (`output_ramp`) → `cfg.out_slew`
...
| | SimpleFOC | shark_pid |
|---|---|---|
| Lọc khâu D | không có | `cfg.d_tau`, hệ số suy từ `dt` |
| Vi phân theo giá trị đo | không có — dùng sai số nên có derivative kick | `cfg.c = 0` |
| Vùng chết | không có | `cfg.deadband` |
| Biến tốc độ tích phân | không có | `cfg.ci_a/ci_b` |
| Feedforward | không có | `cfg.kf`, `cfg.kf_dot`, `update_ff()` |
...
| `OutputFilter` | `cfg.out_tau` *(giây, không phải alpha)* |
| `ChangingIntegralRate` | `cfg.ci_a` / `cfg.ci_b` |
| `DerivativeFilter` | `cfg.d_tau` *(giây, không phải alpha)* |
| `ErrorHandle` | `cfg.stall_time` / `stall_level` / `stall_eps` |
...
Còn `alpha` lọc cũ đổi sang hằng số thời gian: `tau = dt × (1 - alpha) / alpha`.
Với `alpha = 0.25` ở 1 kHz thì `tau = 0.003` giây.
